  ...
  async resolve(parent, args, context) {
    ...
    if (adminAccessToken) {
      ...
      let transporter = nodemailer.createTransport({
        ...
        secure: false,
        auth: {
          user: process.env.RachyBeauty_EMAIL,
          pass: process.env.RachyBeauty_EMAIL_APP_PASSWORD,
        },
        debug: false,
        logger: true,
      });
      ...
      mjmlUtils
        ...
        .then(async (finalTemplate) => {
          await transporter.sendMail({
            from: process.env.RachyBeauty_EMAIL,
            to: employee.email,
            subject: "RachyBeauty Temporary Password",
            html: finalTemplate,
          });
        });
