...
const confirmAppointmentMutation = {
  ...
  async resolve(parent, args, context) {
    ...
    const newNotification = new Notification({
      ...
      originalAssociatedStaffFirstName: selectedAppointment.professional.split(
        " "
      )[0],
      originalAssociatedStaffLastName: selectedAppointment.professional.split(
        " "
      )[1],
    ...
    (
      await Employee.find({
        employeeRole: "Admin",
        firstName: {
          $ne: selectedAppointment.professional.split(" ")[0],
        },
        lastName: { $ne: selectedAppointment.professional.split(" ")[1] },
      })
    ...
    const updatedEmployee = await Employee.findOne(
      {
        firstName: selectedAppointment.professional.split(" ")[0],
        lastName: selectedAppointment.professional.split(" ")[1],
      },
      (err, currentEmployee) => {
        if (currentEmployee) {
          const notificationsObj = updateNotifications(currentEmployee);
    ...
    return {
      ...
      professional: appt_res.professional,
      date: appt_res.date,
